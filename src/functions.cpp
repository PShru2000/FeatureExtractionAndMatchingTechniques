...
// Initialized to compute each execution time 
auto start = std::chrono::high_resolution_clock::now();


// SSD Calculation
// Calculates the Sum of Squared Differences between two images.
// It focuses on a 7x7 region around the center of the images.
...
// Reads and stores paths of image files from a directory 
// We created a separate results folder. The target images and the resultant matches are stored along with details 
// of task Numbers that gives us an idea of what task the target and matches belong to
// Supports .jpg, .png, .ppm, and .tif formats.
...
// You can find the saved images in a folder called results
...
// Generates a 2D histogram from an image 
...
// Computes distance in 2D images
...
// Generates a 3D RGB histogram of an image
...
// Computes Distance in 3D images
// Calculates the intersection of two 3D histograms
...
// Generates a histogram for the top portion of an image
...
// Generates a histogram for the bottom portion of an image
...
// Applies Sobel filter in x-direction to detect horizontal edges
...
// Calculates the magnitude of gradient vectors from Sobel filters
...
// Calculates a histogram based on gradient magnitudes
...
// Calculates a gray-level co-occurrence matrix for texture analysis
...
// Normalizes a co-occurrence matrix to have values between 0 and 1
...
// Calculates texture features such as energy, entropy, contrast, and homogeneity from a co-occurrence matrix
...
// Reads embeddings from a file and stores them in an unordered map
void readEmbeddings(const std::string& filePath, std::unordered_map<std::string, std::vector<float>>& embeddings) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string filename;
        std::getline(iss, filename, ','); // Split at comma delimiter
...
// Computes a histogram from Gabor filter responses of an image
...
// Matches HOG features between two sets of descriptors using FLANN
float matchHOGFeatures(const std::vector<float>& descriptors1, const std::vector<float>& descriptors2) {
    ...
    // Main reason for computing execution time after each task is to understand how fast the devised algorithm 
    // works for large dataset when compared to other algorithms
...
cv::Mat calculateGradientMagnitude(const cv::Mat& src) {
    ...
    // Ensure output is CV_8U for histogram calculation
    cv::Mat grad_normalized;
    cv::normalize(grad, grad_normalized, 0, 255, cv::NORM_MINMAX);
    grad_normalized.convertTo(grad_normalized, CV_8U);
...
// Each tasks performs feature matching using different distances
// Computes distance using different metrics such as SSD , histogramIntersection 2D , histogramIntersection 3D , cosine distance , FLANN
// Sorts the images according to their scores 
// These scores indicate the relative similarity between target images and matches
// In some tasks lesser is the score the more closely it matches to the target images . In few cases its vice versa
// For task == 6 , computing both classical features using RGB histogram and Embeddings using Cosine Distance 
// Aim (task 6) - To give scores next to each other to understand the computational efficiency
...
std::sort(scores.begin(), scores.end(), [](const auto& a, const auto& b) {
    ...
    // Convert target image to grayscale and resize for consistent HOG descriptor size
    cv::Mat grayTarget;
    cv::cvtColor(targetImage, grayTarget, cv::COLOR_BGR2GRAY);
    cv::resize(grayTarget, grayTarget, cv::Size(64, 128)); // Standard size for HOG
    auto targetDescriptors = extractHOGFeatures(grayTarget);

    std::vector<cv::Mat> descriptorMats;
    for (const auto& imagePath : files) {
        cv::Mat image = cv::imread(imagePath, cv::IMREAD_COLOR);
        if (image.empty()) {
            std::cerr << "Failed to read image: " << imagePath << std::endl;
        ...
        // Convert image to grayscale and resize for consistent HOG descriptor size
        cv::Mat grayImage;
        cv::cvtColor(image, grayImage, cv::COLOR_BGR2GRAY);
        cv::resize(grayImage, grayImage, cv::Size(64, 128)); // Standard size for HOG
        auto imageDescriptors = extractHOGFeatures(grayImage);

        // Convert descriptors to Mat
        cv::Mat descriptorMat(imageDescriptors, true); // Convert vector to Mat
        descriptorMats.push_back(descriptorMat);
    }

    // FLANN based matching for each descriptorMat with targetDescriptors
    for (size_t i = 0; i < descriptorMats.size(); ++i) {
        float matchScore = matchHOGFeatures(targetDescriptors, descriptorMats[i]);
        std::string imagePath = files[i];
        scores.emplace_back(imagePath, matchScore);
    }
    ...
    cv::Mat targetImageRGB = targetImage.clone();
    // Ensure targetImage is suitable for histogram calculation
    if(targetImageRGB.channels() != 3) cv::cvtColor(targetImageRGB, targetImageRGB, cv::COLOR_GRAY2BGR);
    
    // Generate RGB and HS histograms for the target image
    cv::Mat targetHistRGB = generateRGBHistogram(targetImageRGB, 8);
    cv::Mat targetHistHS = generateHueSaturationHistogram(targetImageRGB, 8, 8, true); // Adjust for half or full image as needed
    ...
    for (const auto& imagePath : files) {
        ...
        // Generate RGB and HS histograms for each image
        cv::Mat histRGB = generateRGBHistogram(image, 8);
        cv::Mat histHS = generateHueSaturationHistogram(image, 8, 8, true); // Same adjustment as target

        // Calculate histogram intersections
        float distanceRGB = histogramIntersection3D(targetHistRGB, histRGB);
        float distanceHS = histogramIntersection(targetHistHS, histHS);

        // Combine distances for final score
        float combinedScore = (distanceRGB + distanceHS) / 2;

        // Push back combined score
        scores.emplace_back(imagePath, combinedScore);
    }
